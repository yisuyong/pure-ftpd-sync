...
static char *ips_mysql_getquery(MYSQL * const id_sql_server,
                               const char * const orig_query,
                               const char * const account,
                               const char * const ip,
                               const char * const port,
...
{ /* jimmy add */
    ...
    if (mysql_field_count(id_sql_server) != 2) {
        goto bye;
    }
    if ((qresult = mysql_store_result(id_sql_server)) == NULL) {
        goto bye;
    ...
    if (mysql_num_rows(qresult) == 0) {
        logfile(LOG_WARNING, MSG_SQL_WRONG_PARMS " : No servers [%s]", query);
        goto bye;
    }
